...
import { AddCoursePopup } from "./Courses";
...
export default function ShareTheLesson() {
  ...
  const submitHandler = async () => {
    ...
    if (
      ...
      formData.append("nft2Ids", JSON.stringify(list));
      setIsSubmit(false);
      setloader(true);
      axios({
        method: "Post",
  ...
  const updateSelectedCourse = (data) => {
  ...
  return (
    <Box className={classes.LoginBox}>
          ...
          <Box>
            ...
            <Box display="flex" justifyContent="center">
              {isSubmit && list.length === 0 && (
                <FormHelperText error>Please select course</FormHelperText>
              )}
            </Box>
          </Box>
        </Box>
      ...
      {openAddCourse && (
        <AddcoursePopup
          open={openAddCourse}
          handleClose={() => setOpenAddCourse(false)}
          callbackFun={course}
        />
      )}
    </Box>
